...
- `psql \copy ... FROM STDIN`은 데이터 스트림 안에 정확히 `\.`로만 이루어진 줄이 있으면 스트림
  종료로 오인한다. `body_md`/`text`/`raw` 같은 자유 텍스트 컬럼에 그런 줄이 우연히 있을 극단적인
  경우는 다루지 않는다 (`sync_export.sh`/`sync_apply.sh` 둘 다 동일한 전제를 공유).
