...
| 작성일 | 2026-07-18 |
| 갱신 | **2026-07-21 — v1.28: issue_type 세분류 (~40 labels)** |
...
### 완료 스냅샷 (2026-07-21 v1.28)
- **이슈 유형 세분류** (`group_by=issue_type`, ~40 labels):  
  - 성능: 타임아웃 · 성능저하/지연 · 리소스고갈 · 성능테스트/튜닝  
  - 가용성: 접속불가 · 로그인/인증불가 · 서비스장애/FRB · 시스템Crash/Hang · Failover/클러스터  
  - 오류: 소프트웨어버그 · 통신/연동오류 · 작업/명령실패 · 일반오류  
  - 도메인: LB · 방화벽 · DNS · 네트워크일반 · 스토리지IO/용량 · DB성능/장애/구축 · 인증서 · 암호화 · 보안점검  
  - 변경/프로세스: 패치 · 설정오류 · 구성/표준화 · 전환/구축 · 점검 · 모니터링 · 도구개발 · 문의/검토 · 교육/채용/행정  
- **MD→HTML** (v1.26) · 홈 통합 질의 (v1.25)  
...
**문서 끝 (v1.28).**  
이슈 유형 세분류 + MD HTML 렌더 + 통합 질의 홈 · 잔여=사람 도메인 사인·상용 IdP · **매 작업 현행화**.
